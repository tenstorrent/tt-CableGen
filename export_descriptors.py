...
def export_cabling_descriptor_for_visualizer(cytoscape_data: Dict, filename_prefix: str = "cabling_descriptor") -> str:
    ...
    - All structures are now wrapped in proper hierarchy (including "extracted_topology" root)
    """
    ...
    # 2. Graph nodes present (including "extracted_topology" root from mode switching)
    elements = cytoscape_data.get("elements", [])
    has_logical_topology = False
    
    # Check for graph nodes first (including extracted_topology root)
    has_graph_nodes = any(
    ...
    if has_logical_topology:
        ...
        # No graph nodes found - this should not happen as mode switching creates "extracted_topology" root
        raise ValueError(
...
def export_hierarchical_cabling_descriptor(cytoscape_data: Dict) -> str:
    ...
    if not root_graph_nodes:
        # No hierarchical structure found - this should not happen as mode switching creates "extracted_topology" root
        raise ValueError(
    ...
    if use_initial_root:
        ...
        if root_template_name == "extracted_topology":
            root_instance = cluster_config_pb2.GraphInstance()
            root_instance.template_name = root_template_name
